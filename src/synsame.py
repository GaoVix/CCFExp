...
dotenv.load_dotenv(dotenv_path=root / '.env', override=True)
...
import omegaconf
from src.visualizations.extra_visualization import dataset_generate, dataset_test
...
def main():
    ...
    parser.add_argument('--id_images_root', default='')
    parser.add_argument('--style_images_root', type=str, default='')
    ...
    # model_hparam['unet_config']['params']['pretrained_model_path'] = os.path.join(root, os.path.join(root, os.path.join(*model_hparam['unet_config']['params']['pretrained_model_path'].split(os.sep)[4:])))
    model_hparam['unet_config']['params']['pretrained_model_path'] = ''
    model_hparam['recognition']['ckpt_path'] = os.path.join(root, os.path.join(root, os.path.join(
        *model_hparam['recognition']['ckpt_path'].split(os.sep)[4:])))
    ...
    model_hparam['id_model']['ckpt_path'] = os.path.join(root, os.path.join(root, os.path.join(
        *model_hparam['id_model']['ckpt_path'].split(os.sep)[4:])))
    model_hparam['face_model']['ckpt_path'] = os.path.join(root, os.path.join(root, os.path.join(
        *model_hparam['face_model']['ckpt_path'].split(os.sep)[4:])))
    model_hparam['_target_'] = 'src.trainer.Trainer'
    model_hparam['lm_detector_path'] = os.path.join(root, os.path.join(root, os.path.join(
        *model_hparam['lm_detector_path'].split(os.sep)[4:])))
    model_hparam['_partial_'] = True
    # model_hparam['ckpt_path'] = ''
    model_hparam['ckpt_path'] = os.path.join(root, os.path.join(root, os.path.join(
        *model_hparam['ckpt_path'].split(os.sep)[4:])))
    ...
    if os.path.isdir(args.id_images_root):
        ...
        id_images = natural_sort(id_images)
        id_dataset = ListDatasetWithIndex(id_images, flip_color=True)
        assert len(id_dataset) > 0, args.id_images_root
    else:
    ...
    if args.save_root is None:
        ...
        args.save_root = os.path.join(root, 'generated_images', runname_name,
                                      f'id_{id_name}/sty_{args.style_sampling_method}_{style_dir_name}')
    ...
    dataset_test(pl_module, style_dataset, id_dataset,
