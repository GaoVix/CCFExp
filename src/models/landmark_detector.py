...
class MobileFaceNet(Module):
    ...
    def forward(self, x, sign):
        ...
        if sign == 'return_feature':
            return conv_features
        elif sign == 'return_result':
            out = self.output_layer(conv_features)
            B, D = out.shape
            out = out.view(B, 68, 2)
            return out
        else:
            raise RuntimeError("Unknown sign for landmark detection")
...
def load_pretrained_weights(model, checkpoint):
    ...
    print('load_weight for MobileFaceNet', len(matched_layers))
    return model