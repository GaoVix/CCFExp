...
def download_ir_pretrained_statedict(backbone_name, dataset_name, loss_fn):
    ...
    if backbone_name == 'ir_101' and dataset_name == 'webface4m' and loss_fn == 'adaface':
        root = os_utils.get_project_root(project_name='ccfexp')
...
class RecognitionModel(nn.Module):
    ...
    def forward(self, x, orig_images=None):
        ...
        if self.recognition_config.return_spatial:
            ...
            feature, norm = self.backbone(x, return_spatial=self.recognition_config.return_spatial)
        # feature, norm, spatials = self.backbone(x, return_spatial=self.recognition_config.return_spatial)
        # print('norm:', norm)
        # print('feature:', feature)
        if self.recognition_config.normalize_feature:
            feature = feature
            # raise RuntimeError('Feature normalization')
        else:
...
def make_recognition_model(recognition_config, enable_training=False):
    ...
    if recognition_config.ckpt_path:
        if recognition_config.name == 'backbone' or recognition_config.name == 'id_model':
            print(f'loading {recognition_config.name} checkpoint from ')
        ...
        elif recognition_config.name == 'face_model':
            print('making face model')
        else:
            raise NotImplementedError()
    ...
    if recognition_config.name == 'face_model':
        print(f'loading {recognition_config.name} checkpoint from ')
        print(recognition_config.ckpt_path)
        statedict = torch.load(recognition_config.ckpt_path, map_location='cpu')
        del statedict['head.weight']
        del statedict['head.bias']
