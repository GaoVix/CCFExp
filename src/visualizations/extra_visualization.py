...
def style_image_sampler(style_sampling_method, num_image_per_subject, id_index_splits, names_splits, id_dataset,
    ...
    if 'feature_sim' in style_sampling_method:
        ...
        for id_batch in tqdm(id_dataloader, total=len(id_dataloader), desc='inferring id features'):
            id_features, _ = pl_module.recognition_model(id_batch[0].to(pl_module.device))
...
def dataset_generate(pl_module, style_dataset, id_dataset, num_image_per_subject,
    ...
    for batch in tqdm(datagen_dataloader, total=len(datagen_dataloader), desc='Generating Dataset: '):
        ...
        # plotting_images = sample_batch(id_images, style_images, pl_module, seed=None)
...
class StyleIdtestDataset(Dataset):

    def __init__(self, style_dataset, id_dataset):
        super(StyleIdtestDataset, self).__init__()


        self.style_dataset = style_dataset
        self.id_dataset = id_dataset

        assert len(self.style_dataset) == len(self.id_dataset)

    def __len__(self):
        return len(self.style_dataset)

    def __getitem__(self, idx):

        id_images = torch.stack([self.id_dataset[idx][0]])
        style_images = torch.stack([self.style_dataset[idx][0]])
        return id_images, style_images

def dataset_test(pl_module, style_dataset, id_dataset, batch_size=64, num_workers=0, save_root='./', style_sampling_method='random',
                     num_partition=1, partition_idx=0, writer=None):
    os.makedirs(save_root, exist_ok=True)
    print(save_root)


    datagen_dataset = StyleIdtestDataset(style_dataset, id_dataset)

    def collate_fn(data): return data[0]
    datagen_dataloader = DataLoader(datagen_dataset, num_workers=num_workers, batch_size=1, shuffle=False, collate_fn=collate_fn)
    idx = 0
    for batch in tqdm(datagen_dataloader, total=len(datagen_dataloader), desc='Generating Dataset: '):
        id_images, style_images= batch
        plotting_images = sample_batch(id_images, style_images, pl_module, seed=0)
        # plotting_images = sample_batch(id_images, style_images, pl_module, seed=None)
        for image in plotting_images:
            save_name = f"{idx}.jpg"
            idx += 1
            if writer is not None:
                writer.write(image, save_name)
