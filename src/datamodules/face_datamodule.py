...
class FaceDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            train_dataset_path = os.path.join(self.hparams.data_dir, self.hparams.dataset_name, 'train')
            test_dataset_path = os.path.join(self.hparams.data_dir, self.hparams.dataset_name, 'test')
            self.data_train = face_dataset.make_dataset(train_dataset_path,
                                                        deterministic=False,
            ...
            self.data_val = face_dataset.make_dataset(test_dataset_path,
                                                        deterministic=True,
                                                        img_size=self.hparams.img_size,
                                                        return_extra_same_label_samples=self.hparams.return_extra_same_label_samples,
                                                        return_identity_image=self.hparams.return_identity_image,)
