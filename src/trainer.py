...
from src.models.landmark_detector import MobileFaceNet, load_pretrained_weights
from src.models.cond_atten import Condition_Adapter
from src.losses.consistency_loss import calc_identity_consistency_loss, calculate_x0_from_eps, cal_landmark_loss
...
class Trainer(pl.LightningModule):
    ...
    def __init__(self, datamodule=None, unet_config=None, optimizer=None, paths=None, sampler=None, ckpt_path=None,
                 *args, **kwargs):
        ...
        checkpoint = torch.load(self.hparams.lm_detector_path)
        self.landmark_detector = load_pretrained_weights(landmark_detector, checkpoint)
        self.landmark_detector.eval()
        self.landmark_detector.train = partial(disabled_train, self=self.landmark_detector)
        for param in self.landmark_detector.parameters():
            param.requires_grad = False
        self.emotion_adapter = nn.Conv1d(in_channels=1, out_channels=49, kernel_size=1)
        self.condition_adapter = Condition_Adapter(512, 49, num_heads=4, qkv_bias=True, attn_drop=0., proj_drop=0.)
        self.cross_attn_adapter = nn.Conv1d(in_channels=512, out_channels=1024, kernel_size=1)
        self.face_model: RecognitionModel = make_recognition_model(self.hparams.face_model, enable_training=False)
        self.id_model: RecognitionModel = make_recognition_model(self.hparams.id_model, enable_training=False)
    ...
    def shared_step(self, batch, stage='train', optimizer_idx=0, *args, **kwargs):
        ...
        if optimizer_idx == 0:
            ...
            id_loss, _ = calc_identity_consistency_loss(eps=noise_pred,
                                                        timesteps=timesteps,
            ...
            lm_loss = cal_landmark_loss(eps=noise_pred, timesteps=timesteps, noisy_images=noisy_images, images=batch['image'], pl_module=self)
            total_loss = total_loss + lm_loss * self.hparams.losses.landmarks_loss_lambda
            loss_dict[f'{stage}/lm_loss'] =lm_loss

            loss_dict[f'{stage}/total_loss'] = total_loss

            # second loss
            sec_clean_images = calculate_x0_from_eps(noise_pred, noisy_images, timesteps, self.noise_scheduler)
            batch['image'] = sec_clean_images
            sec_noisy_images = self.noise_scheduler.add_noise(sec_clean_images, noise.to(sec_clean_images.device), timesteps.to(sec_clean_images.device))
            sec_encoder_hidden_states = self.get_encoder_hidden_states(batch, batch_size=None)
            sec_noise_pred = self.model(sec_noisy_images, timesteps.to(sec_clean_images.device), sec_encoder_hidden_states).sample
            sec_mse_loss = F.mse_loss(sec_noise_pred, noise.to(sec_clean_images.device))
            sec_loss = sec_loss + sec_mse_loss * self.hparams.losses.mse_loss_lambda
            loss_dict[f'{stage}/sec_mse_loss'] = sec_mse_loss
            sec_id_loss, _ = calc_identity_consistency_loss(eps=sec_noise_pred, timesteps=timesteps,
                                                            noisy_images=noisy_images, batch=batch,
                                                            pl_module=self)
            sec_loss = sec_loss + sec_id_loss * self.hparams.losses.identity_consistency_loss_lambda

            loss_dict[f'{stage}/sec_id_loss'] = sec_id_loss

            sec_lm_loss = cal_landmark_loss(eps=sec_noise_pred, timesteps=timesteps, noisy_images=noisy_images,
                                            images=clean_images, pl_module=self)
            sec_loss = sec_loss + sec_lm_loss * self.hparams.losses.landmarks_loss_lambda
            loss_dict[f'{stage}/sec_lm_loss'] = sec_lm_loss

            loss_dict[f'{stage}/sec_loss'] = sec_loss
            final_loss = total_loss + sec_loss * self.hparams.losses.sec_loss_sigma
            loss_dict[f'{stage}/final_loss'] = final_loss
