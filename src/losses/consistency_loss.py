...
def calc_identity_consistency_loss(eps, timesteps, noisy_images, batch, pl_module, ):
    ...
    if recognition_model.center is not None and pl_module.hparams.losses.identity_consistency_loss_source == 'center':
    ...
    elif pl_module.hparams.losses.identity_consistency_loss_source == 'mix': # selected
        ...
        elif pl_module.hparams.losses.identity_consistency_loss_center_source == 'id_image': # selected
...
def cal_landmark_loss(eps, noisy_images, images, timesteps, pl_module):

    scheduler = pl_module.noise_scheduler
    x0_pred = calculate_x0_from_eps(eps, noisy_images, timesteps, scheduler)
    gen_landmarks = pl_module.landmark_detector(x0_pred, sign='return_result')
    clean_images_landmarks = pl_module.landmark_detector(images, sign='return_result')

    return F.mse_loss(gen_landmarks, clean_images_landmarks) * 10
